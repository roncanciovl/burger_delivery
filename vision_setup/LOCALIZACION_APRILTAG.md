...
- **map / world** (Raíz)
  - $\rightarrow$ **table_link** (Mesa física)
    - $\rightarrow$ **tag_mesa** (Tag de referencia en la esquina)
      - $\dashrightarrow$ *[Detección Visión]* $\dashrightarrow$ **tag_carrito1** (Tag en el techo del carro 1)
        - $\rightarrow$ **car1_base_link** (Centro del Carro 1)
      - $\dashrightarrow$ *[Detección Visión]* $\dashrightarrow$ **tag_carrito2** (Tag en el techo del carro 2)
...
## 2. Instrucciones para tu Nodo de Visión

El módulo de visión que tienes (en Python o C++) necesita estar suscrito a los tópicos de las detecciones de la librería `apriltag_ros` o tu detector custom.
...
class VisionLocalizationNode(Node):
    def __init__(self):
        super().__init__('vision_localization')
...
1. 📌 **Arquitectura TF vs URDF (importante entender la separación):**
   Los frames `tag_carrito1` y `tag_carrito2` **NO existen en el URDF**. Solo existen en el árbol de TF en tiempo de ejecución, publicados dinámicamente por tu nodo de Python.
   El URDF no tiene joints fijos para los carritos. Por defecto el launch **no posiciona los carritos** — es el nodo de visión quien debe hacerlo. Para visualización sin visión activa, usa el parámetro `use_static_carts:=true`.

2. **Launch File**: Lanza el sistema con:
   ```bash
   ros2 launch burger_description display.launch.py
   ```
   Los carritos no aparecerán hasta que el nodo de visión publique la cadena TF `tag_mesa → tag_carrito → car_base_link`. Para modo debug con posición fija:
   ```bash
