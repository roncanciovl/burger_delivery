...
| `burger_perception` | ament_python | Localizador AprilTag: `apriltag_localizer` publica `/<ns>/pose2d` y, con `publish_tf`, el TF `tag_mesa -> tag_carrito1`. Nodo Gemini: `gemini_spatial_reasoning_node` publica el TF `base_link -> target_burger_box_frame` | OpenCV, image_geometry, `google-genai` (pip) |
...
| `use_vlm` | `false` | Nodo Gemini (`burger_perception/launch/gemini.launch.py`); requiere `GEMINI_API_KEY` |
