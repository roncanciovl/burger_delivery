...
| **3. Configuración de CycloneDDS y Monitor de Red:** Justifique por qué se configuró `rmw_cyclonedds_cpp` y cómo `<Interfaces><NetworkInterface>` selecciona la ruta de salida en `cyclonedds.xml` (el antiguo `NetworkInterfaceAddress` no existe en CycloneDDS 0.10) y qué anomalías de red permite detectar el monitor web. | |
...
| ☐ | **N4 — 400–474** | Configura `rmw_cyclonedds_cpp` en ambos equipos, verifica la selección de interfaz de `cyclonedds.xml`, lanza el Monitor de Red (`:8080`) y verifica recepción remota fluida ($\ge 20\text{ Hz}$) en Dispositivo B. |
