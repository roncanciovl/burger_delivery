...
def build():
    ...
    add_list_item(doc, "Clone el driver, aplique el parche de parada limpia y compile kinova_vision con un solo comando: bash ~/ros2_ws/src/burger_delivery/scripts/aplicar_parche_kinova_vision.sh . El script clona ros2_kortex_vision en ~/ros2_ws/src si falta (junto a ros2_kortex, NO dentro de burger_delivery), avisa si falta algún paquete del paso anterior, aplica el parche sin repetirlo, verifica el resultado y compila. Para comprobar sin modificar nada, añada --check. Sin el parche el driver no se detiene limpiamente con Ctrl+C y deja la cámara rechazando streams (TROUBLESHOOTING.md §3.4).", 2)
    add_list_item(doc, "Sin ros-jazzy-image-transport-plugins el nodo solo publica el tópico crudo y /compressed nunca aparece. Verifique con: ros2 run image_transport list_transports", 3)
    add_list_item(doc, "En Dispositivo A, lance el publicador de visión: ros2 launch kinova_vision kinova_vision.launch.py device:=192.168.1.10", 4)
    add_list_item(doc, "Verifique los nodos /camera/kinova_vision_color y /camera/kinova_vision_depth con: ros2 node list | grep kinova_vision", 5)
    add_list_item(doc, "NO mida el tópico crudo con ros2 topic bw: se suscribe en best effort y cada imagen de 6.22 MB viaja en miles de fragmentos (medido: 1.88 MB/s frente a ≈162 MB/s reales). Calcule el crudo como ancho × alto × 3 × FPS, tomando los FPS de ros2 topic hz sobre /camera/color/image_raw/compressed, y mida el comprimido con ros2 topic bw. Registre ambos y el ahorro en la Tabla 3.", 6)
    add_list_item(doc, "Parámetro de calidad JPEG: ros2 param list /camera/kinova_vision_color | grep -i jpeg devuelve .image_raw.compressed.jpeg_quality (con punto inicial, valor inicial 95). Aplíquelo con ros2 param set /camera/kinova_vision_color .image_raw.compressed.jpeg_quality 30 y compare q=80 vs q=30.", 7)
    add_list_item(doc, "Detenga el driver con Ctrl+C y verifique la parada limpia: ambos nodos con 'process has finished cleanly', pgrep -a -x kinova_vision_n vacío y sin líneas ESTAB en ss -tanp | grep 192.168.1.10:554. Un proceso huérfano mantiene la sesión RTSP y bloquea la cámara para las demás estaciones. Si aparecen segfaults o escalado a SIGTERM, compruebe el parche con aplicar_parche_kinova_vision.sh --check.", 8)
