...
Este documento resume los tipos de juntas (joints) disponibles en URDF para ROS 2, su propósito, implicaciones en TF, control con `ros2_control` y ejemplos mínimos. 

> **✨ Animaciones Visuales:** Puedes ver cómo se comporta cada joint abriendo el archivo **`animaciones_joints.html`** (en esta misma carpeta) en tu navegador web.
## Visión general
...
## Ejemplos Visuales
- **Animaciones Web:** Abre `animaciones_joints.html` en tu navegador para interactuar con las animaciones SVG de cada grado de libertad.
- **Robot Real:** El modelo del Kinova Gen3 utilizado en el proyecto principal hace uso de juntas `revolute`, `fixed` y `mimic` (para sincronizar los dedos del gripper).
